...
const API_URL = "PASTE_URL_DEPLOYMENT_APPS_SCRIPT_DISINI";
...
async function apiGet(action) {
    const r = await fetch(API_URL + '?action=' + action, { method: 'GET', redirect: 'follow' });
...
async function initAuth() {
    let status;
    try {
        status = await apiGet('authStatus');
    } catch (err) {
        setLoading(false);
        alert('GAGAL TERHUBUNG KE SERVER!\n\nError: ' + err.message +
              '\n\nPenyebab paling umum:\n1. API_URL di script.js belum diganti dengan URL deployment Apps Script\n2. Apps Script belum di-deploy sebagai Web app\n3. Setting deploy salah (harus: Execute as User, Access: Anyone with Google account)\n\nURL sekarang: ' + API_URL);
        return false;
    }
    if (!status || status.status !== 'success' || !status.email) {
        setLoading(false);
        alert('Tidak bisa membaca akun Google kamu. Pastikan Apps Script di-deploy dengan "Execute as: User accessing the web app".');
        return false;
    }
...
document.getElementById('authForm').addEventListener('submit', async function (e) {
    ...
    btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> <span>' +
        (mode === 'register' ? 'Membuat akun...' : 'Memverifikasi...') + '</span>';
