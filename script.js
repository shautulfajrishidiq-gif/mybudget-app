...
async function initAuth() {
    // 1) Tanya backend siapa yg sedang login (Google account)
    let status;
    try {
    ...
    // 2) Dev: langsung masuk, skip password
    if (currentUser.isDev) {
        try { localStorage.setItem(AUTH_KEY, currentUser.email); } catch(e) {}
        return true;
    }

    // 3) Sudah pernah unlock di browser ini? langsung masuk
    let unlocked = null;
...
// loadData(); // dipanggil oleh startup() setelah auth
